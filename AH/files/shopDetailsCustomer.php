...
?>
<?php
	$stmt0 = $conn->query("SELECT name FROM abedhsen.customer where cid = $cid;");
	$row0=$stmt0->fetch(PDO::FETCH_ASSOC);
	$stmtshop = $conn->query("SELECT shopName,shopCategory,shopEmail,shopContact,shopInfo FROM abedhsen.shop where shopId = $shopId;");
...
<?php
	$image="";
		$dir = 'uploads'; 
		$stmti = $conn->query("SELECT img1 FROM abedhsen.shopimage where shopId = $shopId;");
...
<?php
	$image="";
	$dir = 'uploads'; 
	$stmt = $conn->query("SELECT productName,pid,img1,img2,img3,price FROM abedhsen.product where shopId = $shopId;");
