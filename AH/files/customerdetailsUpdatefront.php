...
<?php
	...
	session_start();
	$cid = $_SESSION['NavCid'];
	$stmt0 = $conn->query("SELECT name,email,contactNo FROM abedhsen.customer where cid = $cid;");
	$stmt1 = $conn->query("SELECT pri_add,home_add,work_add FROM abedhsen.customer_address where cid = $cid;");
